...
requests["Find all the actors who have not acted in any movie between 1990 and 2000"] = "SELECT act_fname, act_lname FROM actors WHERE id NOT in (SELECT act_id FROM movies_actors WHERE mov_id in (SELECT id FROM movies WHERE mov_year BETWEEN 1990 AND 2000));"