...
requests["Find all the actors who have not acted in any movie between 1990 and 2000"] = "SELECT a.act_fname, a.act_lname, f.mov_title, f.mov_year FROM (SELECT * FROM movies AS m INNER JOIN movies_actors AS ma ON m.id = ma.mov_id) AS f INNER JOIN actors AS a ON f.act_id = a.id WHERE f.mov_id NOT in (SELECT id FROM movies WHERE mov_year BETWEEN 1990 AND 2000);"
